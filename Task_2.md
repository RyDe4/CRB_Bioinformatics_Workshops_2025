...
5. To redirect the output of a command to a file, we can use the `>` operator. If we were to run `grep snoRNA Homo_sapiens.GRCh38.113.chr.gtf > Homo_sapiens_snoRNA.GRCh38.113.chr.gtf`, we would obtain a file called Homo_sapiens_snoRNA.GRCh38.113.chr.gtf containing all of the lines of the original GTF file which contain "snoRNA".
